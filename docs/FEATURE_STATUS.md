...
| 黑曜石岩穿越（孫悟空） | ✅ | 孫悟空可穿越但不可停留，`WorldMap.tsx:228-232`，純角色判定無需 Streak |
...
| 神器購買（Coins） | ✅ |
| 消耗品購買（靈石，i1–i10） | ✅ |
| 開運大富翁 骰子買賣 | 🚫 | 後端設定框架已建（MonopolySettings），前端交易 UI 未實裝，課程結束前不上線 |
...
1. 🚫 開運大富翁 玩家端交易 UI（後端設定已完成，擱置）
2. 🚫 Streak ≥ 7 終極技能（大鬧天宮/天蓬真身/琉璃淨土/八部天龍/佛光普照，擱置）
3. ✅ 黑曜石岩穿越（孫悟空可穿越但不可停留，已實裝於 `WorldMap.tsx:228-232`，無需 Streak）
4. 🔒 世界 Boss 系統
